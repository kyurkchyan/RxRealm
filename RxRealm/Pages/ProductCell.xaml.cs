...
public partial class ProductCell
{
    ...
    public ProductCell(IObservable<bool> parentActivator) : base(parentActivator)
    {
        ...
        {
            Debug.WriteLine($"ProductCell activated for {ViewModel?.Name ?? "null"}");
            Disposable.Create(() => Debug.WriteLine($"ProductCell deactivated for {ViewModel?.Name ?? "null"}"))
    ...
    {
        base.OnParentSet();
        Debug.WriteLine($"ProductCell OnParentSet to {Parent?.GetType().FullName ?? "null"}");
    }

    protected override void OnParentChanged()
    {
        base.OnParentChanged();
        Debug.WriteLine($"ProductCell OnParentChanged to {Parent?.GetType().FullName ?? "null"}");
    }
}
